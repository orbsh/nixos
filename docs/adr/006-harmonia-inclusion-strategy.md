...
2. **Workstation（全加）**
   - `modules/presets/workstation.nix` 导入 `../flake-srv/harmonia.nix` 和 `../podman/full.nix`
...
| 主机/集群 | Harmonia | 来源 |
|-----------|----------|------|
| workstations (orbit) | ✅ | `presets/workstation.nix` |
| workstations (team-*) | ✅ | `presets/workstation.nix` |
...
- 新增 Workstation 节点自动获得 harmonia（通过 workstation preset）
- Portable 节点需手动导入
